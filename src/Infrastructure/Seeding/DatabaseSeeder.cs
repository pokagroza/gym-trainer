...
public class DatabaseSeeder
{
    ...
    public void Seed()
    {
        ...
        AddExercises();
        _ctx.SaveChanges(); // ensure exercises exist before category assignment
        AddCategoriesAndAssign();
        AddSplits();
        _ctx.SaveChanges();
    }

    private void AddCategoriesAndAssign()
    {
        // Basic mapping from muscle groups to broader categories (ru localized names kept)
        ...
        var existingNames = _ctx.ExerciseCategories.Select(c => c.Name).ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var catName in map.Values.Distinct())
        {
            if (!existingNames.Contains(catName))
            {
                _ctx.ExerciseCategories.Add(new ExerciseCategory
                {
                    Name = catName,
                    Description = $"Автоматически созданная категория для группы: {catName}"
                });
            }
        }
        _ctx.SaveChanges();
        ...
        // Build lookup
        var categoryLookup = _ctx.ExerciseCategories.ToDictionary(c => c.Name, c => c.Id, StringComparer.OrdinalIgnoreCase);

        // Assign categories to exercises missing assignment
        var toUpdate = _ctx.Exercises.Where(e => e.ExerciseCategoryId == null).ToList();
        foreach (var ex in toUpdate)
        {
            if (map.TryGetValue(ex.MuscleGroup, out var catName) && categoryLookup.TryGetValue(catName, out var cid))
            {
                ex.ExerciseCategoryId = cid;
            }
            else
            {
                // fallback generic category
                const string otherName = "Прочее";
                if (!categoryLookup.TryGetValue(otherName, out var otherId))
                {
                    var other = new ExerciseCategory { Name = otherName, Description = "Категория по умолчанию" };
                    _ctx.ExerciseCategories.Add(other);
                    _ctx.SaveChanges();
                    categoryLookup[otherName] = other.Id;
                    otherId = other.Id;
                }
                ex.ExerciseCategoryId = otherId;
            }
        }
        _ctx.SaveChanges();
    }
